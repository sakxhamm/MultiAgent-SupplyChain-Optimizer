# Multi-Agent Supply Chain Optimizer - Python Components

This directory contains the Python implementation of the Multi-Agent Supply Chain Optimizer framework. It works as a single `uv` workspace containing all project packages, including:

- `packages/autogen-core`: Core interface definitions and reference implementations for agent runtime, model, tool, workbench, memory, and tracing.
- `packages/autogen-agentchat`: Single and multi-agent workflows for supply chain coordination and optimization.
- `packages/autogen-ext`: Ecosystem integrations and extensions. For example, `autogen-ext[openai]` provides the OpenAI model client.
- `packages/autogen-studio`: Web-based IDE for building and running supply chain optimization agents.
...
`uv` is a package manager that assists in creating the necessary environment and installing packages for the Multi-Agent Supply Chain Optimizer.
...
To do so, create a virtual environment where the project packages are installed based on the current state of the directory.\
